...
def view_apartments(apartments_in_building)
  apartments_in_building.each do |unit|
...
apartments_in_building = []
 ...
 0.upto(apt_count - 1) do |unit|
  ...
  apartments_in_building << apartment
  puts "Apartment #{unit} added successfully \n #{apartment}"
  puts "Is unit #{apartment.name} rented? (y)es (n)o"
  unit_rented = gets.strip
  if unit_rented == "y"
    apartment.renter = create_renter
    puts "Renter #{apartment.renter} added successfully"
...
if response == 'v'
  puts "---------------------- #{building.building_name} Apartment List ------------------------"
  view_apartments(apartments_in_building)
