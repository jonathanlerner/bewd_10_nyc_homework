...
class SecretNumberGame
  ...
  def play
    greeting
    @guesses.times do |x|
      guesses_left = 3.downto(1) #Find a way to decrement guesses_left each time through the loop.
  ...
  def greet(player_name)
    puts "Hello, #{player_name}, let's play the secret number game." #Create a string to use as your greeting. It should include the player_name and the rules, with the number of guesses the player gets.
  end

  def created_by
    SecretNumberGame.created_by = @first_name && @last_name #Created_by is a string that includes your first and last name
  end


  def correct?(players_guess)
    if correct?(players_guess)
